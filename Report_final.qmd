...
Zbiór danych [Credit Card Churn Prediction](https://www.kaggle.com/datasets/anwarsan/credit-card-bank-churn/data) pochodzi z serwisu Kaggle. Zawiera informacje o 10127 klientach banku, u których na podstawie 19 zmiennych określono fakt posiadania przynajmniej jednej karty kredytowej - jest to 20 zmienna w zbiorze.
...
4. `Total_Relationship_Count` - *zmienna ilościowa* opisująca ile kart kredytowych posiada dany klient
...
```{r Podzia zbioru i receptury, eval=FALSE, echo=TRUE}
set.seed(420)
...
trees_wflow <- trees_wflow %>% 
  option_add(param_info = params_dec_tree, id = "simple_dec_tree") %>% 
  option_add(param_info = params_rf, id = "simple_random_forest") %>% 
  option_add(param_info = params_bag, id = "simple_bagging")
...
xgb_lr_wflow <- xgb_lr_wflow %>% 
  ...
  option_add(param_info = params_log_reg, id = "middle_log_reg")
...
```{r, eval=FALSE}
saveRDS(bayes_results, file = 'output/bayes_results.rds')
```

```{r}
bayes_results <- readRDS(file = 'output/bayes_results.rds')
```

```{r Plot Bayes results, echo=TRUE}
# bayes_results %>% 
#   rank_results() %>% 
